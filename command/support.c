...
GLOBAL VOID disp_filetime( unsigned time )
{
   ...
   printf( "%2d%c%02d%c", h, country.dtime[0], /* print hour, delimiter */
...
GLOBAL VOID disp_systime( VOID )
{
   ...
   ms_gettime( &time );
   printf( "%2d%c%02d%c%02d.%02d", time.hour, b, /* print hour, delimiter */
...
GLOBAL VOID revon( VOID ) { screen( REVON_KEY, REVON_DEF ); }

GLOBAL VOID revoff( VOID ) { screen( REVOFF_KEY, REVOFF_DEF ); }
...
{
   if ( b == 0x8D ) {
      return ( 'i' ); /* For turkish dotted capital I */
   }
...
GLOBAL WORD dr_strnicmp( REG BYTE *str1, REG BYTE *str2, UWORD len )
{
   ...
   {
      if ( dbcs_lead( *str1 ) ||
           dbcs_lead( *str2 ) ) { /* one or both characters are DBCS */
...
{
   if ( is_filechar( s ) || *s == *pathchar || *s == '.' || *s == ':' ||
        ( *s >= 'Z' + 1 && *s <= 'Z' + 6 ) ) {
 ...
 */
GLOBAL BYTE *get_filename( REG BYTE *newpath, REG BYTE *oldpath,
                           BOOLEAN ambiguous )
{
   UWORD count = 0;
...
GLOBAL VOID repwild( REG BYTE *src, REG BYTE *dest )
{
   ...
   strcpy( temp,
           dest ); /* copy wild dest to temp, as dest will get overwritten */
...
GLOBAL VOID crlf( VOID ) { printf( "\n" ); }

GLOBAL VOID putc( BYTE c ) { printf( "%c", c ); }

GLOBAL VOID puts( BYTE *s ) { printf( "%s", s ); }
...
GLOBAL WORD e_check( REG WORD ret )
{
   ...
   case ED_ENVIRON:
      s = MSG_ENVERR;
      break; /* Invalid Environment	*/
   case ED_DRIVE:
      s = ERR15;
      break; /* Invalid Drive Spec	*/
   case ED_PROTECT:
      s = ERR19;
      break; /* Write Protect Disk	*/
   case ED_SHAREFAIL:
      s = ERR20;
      break; /* Sharing Conflict	*/
   default:
      if ( ret == ED_FAIL ) {
         s = ERR83;
         break;
      } /* Physical Media - FAILED */
      if ( ret == ED_PASSWORD ) {
         s = ERR86;
         break;
      } /* Invalid Password	*/
...
GLOBAL BOOLEAN f_check( REG BYTE *cmd, BYTE *fchars, UWORD *farray,
                        BOOLEAN ignore )
   ...
   *farray = 0; /* assume none of the flags present */
   ...
   while ( *cmd ) {
      ...
      FOREVER
      {
         ...
         if ( ( s = (BYTE *)strchr( fchars,
                                    c ) ) ) { /* check each char against  */
...
GLOBAL BOOLEAN nofiles( REG BYTE *path, WORD attrib, BOOLEAN exist,
...
/* exist: Must files exist	*/
{
   ...
   if ( ret < 0 ) { /* Check the error returned  */
      if ( !exist && ( ret == ED_FILE ||
                       ret == ED_ROOM ) ) { /* If file does not exist but*/
...
GLOBAL VOID optional_line( BYTE *line )
{
   ...
   else if ( boot_key_scan_code == 0x4200 ) {
   ...
   while ( !( ( yn == YES_CHAR ) || ( yn == NO_CHAR ) ||
              ( ( yn == RUN_CHAR ) && ( boot_key_scan_code == 0x4200 ) ) ) ) {
      yn = (BYTE)msdos( ( MS_C_RAWIN ), NULL ); /* read the response	*/
   ...
   /*	if (!yes(NO,NO)) *line = 0;*/
   if ( yn == NO_CHAR ) {
      *line = 0; /* skip this line if NO */
   }
   if ( yn == RUN_CHAR ) {
      boot_key_scan_code = 0; /* run-through, deselect F8 key */
   }
