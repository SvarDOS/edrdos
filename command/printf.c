...
GLOBAL WORD CDECL printf( BYTE *fmt, UWORD args )
...
GLOBAL WORD CDECL eprintf( BYTE *fmt, UWORD args )
{
   UWORD totlen, len;
   BYTE local_buff[130];
...
GLOBAL WORD CDECL fprintf( UWORD handle, BYTE *fmt, UWORD args )
{
   UWORD totlen, len;
   UWORD bufsize;
...
GLOBAL WORD CDECL sprintf( BYTE *strg, BYTE *fmt, UWORD args )
{
   buff = (BYTE FAR *)strg;               /* Point buffer at String	*/
   return _printf( fmt, &args, SPRINTF ); /* Generate the String		*/
}

MLOCAL UWORD CDECL _printf( BYTE *fmt_near, UWORD *args, UWORD type )
{
   BYTE FAR *fmt;
...
MLOCAL VOID arg_ptrs( REG BYTE FAR *fmt, UWORD *args, UWORD *ptr_list[] )
/* fmt: ASCIIZ format string */
/* args: pointer to first argument */
/* ptr_list: list of pointers to all arguments */
{
...
MLOCAL BYTE FAR *arg_siz( REG BYTE FAR *fmt, BYTE *siz )
/* fmt: printf argument format string */
/* siz: returned argument size, in bytes */
{
   ...
   *siz = 0; /* argument size not yet known */
   ...
   if ( *fmt == '*' ) {       /* width argument also on stack */
   ...
   if ( *fmt == '.' ) {
      fmt++;
      if ( *fmt == '*' ) { /* 2nd width argument on stack */
         *siz += sizeof( WORD );
         fmt++;
      }
      else { /* ignore any static width control */
...
MLOCAL BYTE FAR *arg_num( BYTE FAR *fmt, UWORD *num )
{
   REG BYTE FAR *cp;
   REG UWORD n;
...
MLOCAL BYTE *to_hex( ULONG n, BYTE *s, BYTE hex_base )
/* hex_base: 'a'-10 for lowercase, 'A'-10 uppercase */
{
   REG UBYTE i;
...
MLOCAL BYTE *to_dec( ULONG n, BYTE *s )
{
   if ( (LONG)n < 0 ) {
...
MLOCAL BYTE *to_udec( ULONG n, BYTE *s )
{
   do {
      *--s = '0' + ( n % 10 );
      n /= 10;
