 ...
 *   type D:TEST, it will look for TEST on the current directory of drive D:,
then
 ...
 *   message for DIR being an incorrect loadpath, tries to access A: and
crashes
 *   under /MULTI.
 *   Break added after error message to fix crashing problem.
 *   Variable lferror added so that after an incorrect loadpath, it does not
use
 ...
 *   /MU option only valid with /P option. The problem is if you "exit" with
upper
 *   memory unlinked the resident portion isn't freed, and you eat upper
memory.
...
MLOCAL VOID init( BYTE *cmd )
{
   ...
   envsize = oldenvsize;
   if ( ( envsize < 128 ) || ( envsize > 32752 ) ) {
      envsize = 256; /* shouldn't really need this */
   }
   ...
   for ( ret = 5; ret < 20; ms_x_close( ret++ ) )
      ; /* close handles 5-20	     */
   ...
   if ( oldenvsize == 0 ) {
      cflag |= 1; /* Assume primary shell if no previous env */
   }
...
/*RG-00-end*/
{
   ...
   case IA_BREAK:
      ...
      /*		ms_x_close(5);			 ##jc## Close Unused
     * Handles*/
      /*		ms_x_close(6);			 ##jc## Close Unused
     * Handles*/
...
/* internal: Search for INTERNAL Commands */
{
   ...
   for ( i = 0; i < 12; loadfile[i++] = ' ' )
      ;                           /* initialise with blanks */
   ...
   for ( ; is_filechar( cp ); cp++ )
      ; /* skip the command itself */
        /* offer command line to */
        /* interested parties */
   if ( docmd_offer( lcp, loadfile,
                     0xFF00 + strlen( cp ) ) ) { /* If they want it */
      return;                                    /* we don't */
   }
   ...
   while ( internal && s_cmd_p->cmnd ) { /* while more builtins	*/
      ...
      if ( !strncmp( argv0, loadfile + 1, 8 ) ) { /* Is this a match ? */
         ...
         if ( s_cmd_p->needparm != PARAM_NONE /* if a parameter is needed */
            ...
            switch (
               (UWORD)s_cmd_p->needparm ) /* display an error message  */
            {
            case PARAM_NEEDFILE:
               eprintf( MSG_NEEDFILE );
               break;
            case PARAM_NEEDPATH:
               eprintf( MSG_NEEDPATH );
               break;
            case PARAM_NEEDDEV:
               eprintf( MSG_NEEDDEV );
               break;
   ...
   if ( docmd_offer( lcp, loadfile,
                     strlen( cp ) ) ) { /* offer command line to */
...
/* loadtype: Command file Type */
{
   ...
   if ( ( ftype = strchr( fname, '.' ) ) !=
        0 ) {          /* then extract the optional*/
...
#if defined( PASSWORD )
   if ( ( pwd = strchr( pwd, *pwdchar ) ) !=
        0 ) {        /* Finally extract the      */
...
MLOCAL WORD checkfile( BYTE *loadpath, UWORD *loadtype, BYTE *path,
                       BYTE *fname, BYTE *ftype, BYTE *pwd )
{
   UWORD type;
   WORD ret;
   DTA search;
...
MLOCAL BOOLEAN doexec( BYTE *argv0, BYTE *loadpath, UWORD loadtype,
                       BYTE *tail )
...
printf("DOEXEC Load File \"%s\" Command Line \"%s\"\n",
   ...
   */
   ...
   for ( i = 5; i < 20; i++ ) {
      ms_x_close( i ); /* Close all handles */
   }
...
GLOBAL VOID process_config_env( VOID )
{
   ...
   if ( config_env ) {
      ...
      {
         i = 0;
         while ( ( *config_env ) && ( *config_env != 0x1A ) && ( i < 127 ) ) {

            buff[i++] = *config_env++;
         }
         if ( i == 0 ) {
            while ( *config_env != 0x1A ) {
               config_env++;
            }
            config_env++;
